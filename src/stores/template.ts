...
export const useTemplateStore = defineStore("template", {
	...
	actions: {
		...
		sidebar(payload: { mode: "open" | "close" | "toggle" }) {
		...
		sidebarMini(payload: { mode: "on" | "off" | "toggle" }) {
			if (window.innerWidth > 991) {
				if (payload.mode === "on") {
		...
		sidebarPosition(payload: { mode: "left" | "right" | "toggle" }) {
		...
		sidebarStyle(payload: { mode: "dark" | "light" | "toggle" }) {
		...
		sideOverlay(payload: { mode: "open" | "close" | "toggle" }) {
			if (payload.mode === "open") {
		...
		sideOverlayHover(payload: { mode: "on" | "off" | "toggle" }) {
			if (payload.mode === "on") {
		...
		pageOverlay(payload: { mode: "on" | "off" | "toggle" }) {
			if (payload.mode === "on") {
		...
		header(payload: { mode: "fixed" | "static" | "toggle" }) {
		...
		headerStyle(payload: { mode: "dark" | "light" | "toggle" }) {
			if (payload.mode === "dark") {
		...
		headerSearch(payload: { mode: "on" | "off" | "toggle" }) {
			if (payload.mode === "on") {
		...
		headerLoader(payload: { mode: "on" | "off" | "toggle" }) {
			if (payload.mode === "on") {
		...
		pageLoader(payload: { mode: "on" | "off" | "toggle" }) {
			if (payload.mode === "on") {
		...
		setColorTheme(payload: { theme?: string }) {
			const lHtml = document.documentElement;
