...
export const registerPlugins = (app: App) => {
	AppUtils.logger.info("Registering plugins...");
	const imports = import.meta.glob<{ default: (app: App) => void }>(
		["../plugins/*.{ts,js}", "../plugins/*/index.{ts,js}"],
		{ eager: true }
	);

	AppUtils.logger.info(`Found ${Object.keys(imports).length} plugins to register.`);

	const importPaths = Object.keys(imports).sort();

	importPaths.forEach((path) => {
		AppUtils.logger.info(`Registering plugin: ${path}`);
